...
from object_tracker.process_frame_2 import process_frames 
...
class ImageViewer(Node):
    ...
    def image_callback(self, msg):
        ...
        if self.lateral_command == "MOVE LEFT":
            print(f"Moving left with distance factor: {self.distance_factor}")
            int_msg.data = -self.distance_factor
        elif self.lateral_command == "MOVE RIGHT":
            print(f"Moving right with distance factor: {self.distance_factor}")
            int_msg.data = self.distance_factor
        else:
            int_msg.data = 0.0 # HOVER
